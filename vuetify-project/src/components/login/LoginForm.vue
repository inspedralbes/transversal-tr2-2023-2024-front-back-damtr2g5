<template>
    ...
    <v-sheet
        class="round-border myfont bitter-sweet d-flex align-center justify-center flex-wrap text-center mx-auto h-auto pa-9"
        elevation="4">
        <div>
            ...
            <form>
                ...
                <v-text-field variant="underlined" @input="handleHashing($event.target.value)"
                    :append-icon="show0 ? '$eye' : '$eyeOff'" :type="show0 ? 'text' : 'password'" label="Contrasenya"
            ...
            <p class="mt-3 black-olive"> Encara no t'has registrat? <a href="#" class="lavender-pink"
                    @click.stop.prevent="dialog = true, step = 1"> Registra't </a></p>
            <v-dialog v-model="dialog" class="w-50 myfont">
                <v-card class="mx-auto" max-width="800" width="500">
                    <v-card-title class="myfont big-font justify-space-between">
                        {{ currentTitle }}
                        <v-avatar class="text-h6 bitter-sweet-bg mb-3" style="color: white;" size="20"
                    ...
                    <v-window v-model="step">
                        <v-window-item class="bitter-sweet" :value="1">
                            <v-card-text>
                                <v-text-field variant="outlined" label="Email" placeholder="ejemplo@ejemplo.com"
                                    v-model="emailD"></v-text-field>
                                <v-row>
                                    <v-col cols="6">
                                        <v-text-field variant="outlined" label="Nom"
                                            v-model="username"></v-text-field></v-col>
                                ...
                                <v-card-text>
                                    <v-row>
                                        <v-text-field variant="outlined" :append-icon="show1 ? '$eye' : '$eyeOff'"
                                            ...
                                            @click:append="show1 = !show1" v-model="passwordD"></v-text-field></v-row>
                                    <v-row>
                        ...
                        <v-window-item :value="3">
                            <v-container>
                                <v-img
                                    src="https://i.pinimg.com/originals/a7/01/b4/a701b416f376c51673ffe421d2b2d7b0.jpg"></v-img>
                            </v-container>
                        </v-window-item>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn class="oxford-blue-bg bitter-sweet"
                            v-if="step == 2 && this.passwordD == this.passwordComprobacion && this.passwordD != ''"
                            color="primary" variant="flat" @click="register()">
                            Registrar-se
                        </v-btn>
...
<style scoped>.small-dialog .v-dialog__content {
    width: 50%;
