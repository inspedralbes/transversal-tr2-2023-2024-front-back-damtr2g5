<template>
  ...
  <v-expansion-panels class="myfont">
    ...
    <v-expansion-panel class="round-border violet-bg" expand-icon="$plus" collapse-icon="$minus">
      ...
      <v-expansion-panel-text class="platinum-bg" style="border-bottom-left-radius: 40px !important; border-bottom-right-radius: 40px !important;">
        <v-form class="mt-6" @submit.prevent="createRoom">
          <v-text-field variant="outlined" bg-color="white" density="compact" rounded :rules="rules" label="Nom de la partida" v-model="name"></v-text-field>
          <v-row>
            <v-col cols="6" sm="6" md="3" lg="3">
              <v-checkbox label="Sala privada" v-model="privateRoom"></v-checkbox>
            </v-col>
            <v-col cols="6" sm="6" md="3" lg="3">
              <v-select variant="solo-filled"  bg-color="white" label="Tipus de partida" v-model="gameType" :items="gameTypes"></v-select>
            </v-col>
          </v-row>
          <v-text-field v-if="privateRoom" :rules="rules" variant="outlined" bg-color="white" density="compact" rounded label="Contrasenya" v-model="password"></v-text-field>
